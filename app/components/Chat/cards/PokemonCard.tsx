"use client";

import { useState } from "react";
import type { PokemonCardPayload } from "@/lib/agents/types";
import { Lightbox } from "./Lightbox";

// --- Type color map ---

const TYPE_COLOR_MAP: Record<string, string> = {
  fire: "#F08030", water: "#6890F0", grass: "#78C850", electric: "#F8D030",
  psychic: "#F85888", ice: "#98D8D8", dragon: "#7038F8", dark: "#705848",
  fairy: "#EE99AC", normal: "#A8A878", fighting: "#C03028", flying: "#A890F0",
  poison: "#A040A0", ground: "#E0C068", rock: "#B8A038", bug: "#A8B820",
  ghost: "#705898", steel: "#B8B8D0",
};

const STAT_LABELS: Record<string, string> = {
  hp: "HP", attack: "ATK", defense: "DEF",
  "special-attack": "SP.A", "special-defense": "SP.D", speed: "SPD",
};

interface PokemonCardProps {
  data: PokemonCardPayload;
}

export function PokemonCard({ data }: PokemonCardProps) {
  const { name, id, sprite, types, stats, abilities, height, weight } = data;
  const [lightboxOpen, setLightboxOpen] = useState(false);

  return (
    <>
      <div
        className="shrink-0 rounded-2xl overflow-hidden border flex flex-col"
        style={{
          width: 300,
          borderColor: "var(--border-default)",
          background: "var(--surfaces-base-primary)",
        }}
      >
        {/* Image (top, full width, click → lightbox) */}
        <button
          onClick={() => setLightboxOpen(true)}
          className="relative w-full aspect-square flex items-center justify-center cursor-zoom-in"
          style={{ background: "var(--surfaces-base-low-contrast)" }}
          aria-label={`View ${name} fullscreen`}
        >
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={sprite}
            alt={name}
            className="w-[85%] h-[85%] object-contain"
            loading="lazy"
          />
        </button>

        {/* Body */}
        <div className="flex flex-col gap-3 p-4">
          {/* Name + dex number */}
          <div className="flex items-baseline justify-between gap-2">
            <h3
              className="text-lg font-semibold capitalize truncate"
              style={{ color: "var(--typography-primary)" }}
            >
              {name}
            </h3>
            <span
              className="text-sm tabular-nums"
              style={{ color: "var(--typography-secondary)" }}
            >
              #{String(id).padStart(3, "0")}
            </span>
          </div>

          {/* Type badges */}
          <div className="flex flex-wrap gap-1.5">
            {types.map(type => (
              <span
                key={type}
                className="px-2.5 py-0.5 rounded-full text-xs font-semibold text-white capitalize"
                style={{ backgroundColor: TYPE_COLOR_MAP[type.toLowerCase()] ?? "#A8A878" }}
              >
                {type}
              </span>
            ))}
          </div>

          {/* Stats */}
          <div className="flex flex-col gap-1">
            {stats.map(stat => (
              <div key={stat.name} className="flex items-center gap-2">
                <span
                  className="text-[10px] font-semibold uppercase w-10 shrink-0"
                  style={{ color: "var(--typography-secondary)" }}
                >
                  {STAT_LABELS[stat.name] ?? stat.name.slice(0, 3).toUpperCase()}
                </span>
                <span
                  className="text-xs tabular-nums w-8 text-right shrink-0"
                  style={{ color: "var(--typography-secondary)" }}
                >
                  {stat.value}
                </span>
                <div
                  className="flex-1 h-1.5 rounded-full overflow-hidden"
                  style={{ background: "var(--surfaces-base-low-contrast)" }}
                >
                  <div
                    className="h-full rounded-full"
                    style={{
                      width: `${Math.min(100, (stat.value / 255) * 100)}%`,
                      backgroundColor: stat.value >= 100 ? "#78C850" : stat.value >= 60 ? "#F8D030" : "#F08030",
                    }}
                  />
                </div>
              </div>
            ))}
          </div>

          {/* Abilities */}
          {abilities.length > 0 && (
            <p
              className="text-xs leading-relaxed"
              style={{ color: "var(--typography-secondary)" }}
            >
              <span className="font-semibold">Abilities: </span>
              {abilities.map(a => a.replace(/-/g, " ")).join(", ")}
            </p>
          )}

          {/* Footer: height + weight */}
          <div
            className="flex gap-4 text-xs pt-2 border-t"
            style={{ borderColor: "var(--border-default)", color: "var(--typography-secondary)" }}
          >
            <span>Height: {height.toFixed(1)}m</span>
            <span>Weight: {weight.toFixed(1)}kg</span>
          </div>
        </div>
      </div>

      <Lightbox
        src={lightboxOpen ? sprite : null}
        alt={name}
        onClose={() => setLightboxOpen(false)}
      />
    </>
  );
}
